...
## Versión 4 — Consulta semanal por docente

Se añadió una segunda opción principal llamada **“Consultar horas libres de un docente”**.

Características:

- Lista desplegable construida directamente desde `data.js`.
- Tabla semanal de lunes a viernes.
- Cada franja se clasifica como:
  - Disponible
  - En clase
  - Por confirmar
  - Fuera de jornada
- En los bloques `En clase` se muestra sección, curso y asignatura.
- Usa las mismas reglas de disponibilidad que el resto del portal.
- La vista de escritorio usa tabla; en celular cambia automáticamente a tarjetas por día.
- Sigue siendo un sitio 100 % estático: no requiere servidor, Node, base de datos ni librerías externas.


## Versión 5 — corrección de las franjas de la consulta semanal

La consulta **“Consultar horas libres de un docente”** ya no construye intervalos a partir
de todos los límites horarios. Esa lógica producía segmentos artificiales de 5 minutos
cuando se superponían horarios de distintas secciones.

Ahora:

- `data.js` contiene `scheduleBlocks` con las franjas lectivas reales de Preescolar,
  Primaria y Bachillerato.
- `data.js` también contiene `institutionalBlocks` para documentar recreos, descansos,
  almuerzo, oración, izada, agenda/dirección de grupo y salida.
- La consulta semanal usa **solo `scheduleBlocks`**.
- Cada fila corresponde a un bloque lectivo real completo; nunca se subdivide.
- Las pausas institucionales no aparecen como bloques `Disponible`.
- Para un profesor compartido entre secciones se reúnen únicamente los bloques de las
  secciones en las que figura asignado.
- La consulta rápida por día y hora se conserva sin cambios.
