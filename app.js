(() => {
  ...
  function buildTeacherTimeGrid(t){
    // Los límites de la cuadrícula se construyen con todos los inicios/finales relevantes:
    // jornada + eventos exactos/tentativos. Así funciona aunque las secciones tengan bloques diferentes.
    const boundaries = new Set();
    if(t.labor){
      boundaries.add(t.labor.start);
      boundaries.add(t.labor.end);
    }

    [...D.exactEvents, ...D.tentativeEvents].forEach(e=>{
      const applies = ("teacher" in e && e.teacher===t.name) ||
                      ("teachers" in e && e.teachers.includes(t.name));
      if(applies){
    ...
    // Añadir los bloques académicos usados por el colegio para producir una tabla legible
    // aun cuando el profesor no tenga clase en una franja.
    [
      "08:00","08:05","08:10","08:30","08:45","08:50","08:55",
      "09:15","09:25","09:35","09:40","10:00","10:05","10:10",
      "10:25","10:30","10:35","10:55","11:00","11:20","11:40",
      "12:05","12:20","12:25","12:35","12:50","13:10","13:15",
      "13:30","13:55","14:00","14:15","14:35","15:00","15:15",
      "15:40","16:00"
    ].forEach(x=>boundaries.add(x));

    return [...boundaries]
      .sort((a,b)=>mins(a)-mins(b))
      .slice(0,-1)
      .map((start,i,arr)=>{
        const sorted=[...boundaries].sort((a,b)=>mins(a)-mins(b));
        return {start, end: sorted[sorted.indexOf(start)+1]};
      })
      .filter(r=>r.end && mins(r.end)>mins(r.start));
  }
  ...
  function statusForInterval(t,day,start,end){
    const labor=t.labor && t.labor.days.includes(day);
    if(!labor || mins(end)<=mins(t.labor.start) || mins(start)>=mins(t.labor.end)){
    ...
    // Si el intervalo toca parcialmente el inicio/fin de jornada, se considera fuera
    // para no mostrar como libre un tramo que no está completamente dentro de la jornada.
    if(mins(start)<mins(t.labor.start) || mins(end)>mins(t.labor.end)){
      return {kind:"off",label:"Fuera de jornada",events:[]};
    }
  ...
  function cellDetail(status){
    if(status.kind==="busy"){
      return status.events.map(e=>{
        const extra=e.section ? `${e.section} · ` : "";
        return `${extra}${e.course} · ${e.subject}`;
      }).join("<br>");
    }
    if(status.kind==="maybe"){
      return status.events.map(e=>`${e.course} · ${e.subject}`).join("<br>");
    }
    return status.kind==="free" ? "Sin clase identificada" : "No está en jornada";
  }
  ...
  function renderFreeTeacherWeek(){
    ...
    const ranges=buildTeacherTimeGrid(t).filter(r=>{
      // Limitar visualmente a la franja global del colegio
      return mins(r.end)>mins("08:00") && mins(r.start)<mins("16:00");
    });

    let counts={free:0,busy:0,maybe:0,off:0};
    const matrix = ranges.map(r=>{
      const row={range:r,days:{}};
      DAYS.forEach(day=>{
        const s=statusForInterval(t,day,r.start,r.end);
        row.days[day]=s;
        counts[s.kind]++;
      });
      return row;
    ...
    freeTeacherBody.innerHTML=matrix.map(row=>`
      <tr>
        <td class="time-col">${esc(fmt(row.range.start))}<br>${esc(fmt(row.range.end))}</td>
        ${DAYS.map(day=>{
          const s=row.days[day];
          return `<td>
            <div class="free-cell free-cell--${s.kind}">
              <strong>${esc(s.label)}</strong>
              <small>${cellDetail(s)}</small>
            </div>
          </td>`;
        }).join("")}
      </tr>
    ...
    freeTeacherMobile.innerHTML=DAYS.map(day=>`
      <article class="mobile-day">
        ...
        <div class="mobile-day__list">
          ${matrix.map(row=>{
            const s=row.days[day];
            return `<div class="mobile-slot">
              <div class="mobile-slot__time">${esc(fmt(row.range.start))}<br>${esc(fmt(row.range.end))}</div>
              <div class="free-cell free-cell--${s.kind}">
                <strong>${esc(s.label)}</strong>
                <small>${cellDetail(s)}</small>
              </div>
